...
public class PathGrid : MonoBehaviour
{
    ...
    public Node GetNodeFromWorldPoint(Vector3 position)
    {
        ...
        percent.y = Mathf.Clamp01((position.y + worldSize.y / 2) / worldSize.y);
    ...
    void CreatePathGrid()                          //Populacja kratki
    {
        ...
        for(int x = 0; x < gridSize.x; x++)
        {
            for(int y = 0; y < gridSize.y; y++)
            {
                ...
                bool traversable = !(Physics2D.OverlapCircle(worldPoint, nodeSize, traverseMask));
                pathGrid[x, y] = new Node(traversable, worldPoint, new Vector2Int(x, y));   //Ważne, ustalanie czy dane miejsce jest możliwe do pokonania
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (pathGrid != null)
        {
            foreach (Node node in pathGrid)
            {
                ...
                Gizmos.DrawCube(new Vector3(node.worldPosition.x, node.worldPosition.y, 0), Vector3.one * (nodeSize * 2 - .04f));
            }
        }
    }
}
